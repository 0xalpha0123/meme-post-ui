...
interface ButtonBaseProps extends DefaultProps {
  ...
  size?: Sizes;
  variant?: "link" | "solid" | "outline" | "light" | "transparent";
  rounded?: true | false;
  onClick?: MouseEventHandler;
}

const setColor = (color: string | undefined, variant: string) => {
  if (variant === "outline") {
    if (color === "red")
  ...
  if (variant === "transparent") {
    return "border-none bg-transparent dark:bg-transparent hover:bg-transparent hover:dark:bg-transparent text-primary_white-800 dark:text-primary_dark-800";
  }
  if (color === "gray")
...
>({
  ...
  return (
    <Element
      ...
      className={cx(
        className,
        "w-auto inline-flex items-center justify-center space-x-1 font-medium transition border cursor-pointer select-none focus:transition-none focus:outline-none focus:ring-0",
